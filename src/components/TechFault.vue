<template>
    <section class="page page--no-padding error separator-section separator-section__picture full separator-section__full-height separator-section__error">
        <div class="container main-content">
            <div class="column--flex-wrap">
                <div class="column column--12-col">
                    <div class="info-box">
                        <div class="info-box__content">
                            <h1 class="info-box__title info-box__title--white">
                                <div class="info-box__line  info-box__title--large">
                                    <span class="info-box__line__content font-effect-wallpaper">Houston,</span>
                                </div>
                                <div class="info-box__line">
                                    <span class="info-box__line__content font-effect-wallpaper">We have a problem!</span>
                                </div>
                            </h1>
                            <p class="info-box__sub-title info-box__sub-title--white">Something has gone technically wrong.</p>
                            <p class="info-box__sub-title info-box__sub-title--white">But don't worry, our engineers will try and solve the problem with some gaffa tape, an air filter and the cover off the flight plan.</p>
                            <p class="info-box__sub-title info-box__sub-title--white">You can go <a href="javascript:history.back()" target="_self" title="go back" class="info-box__link">back</a> to where you were or you can try heading back <a href="/" target="_self" title="Return to jamiederycke.co.uk" data-ga-category="Error" data-ga-action="Click" data-ga-label="Return home" class="info-box__link">home</a>.</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
</template>
